...
public sealed class FastObjImporter
{
	...
	{
		GameObject returnGO = new GameObject ("Surface model");
		...
		for (int i = 0; i <= faceData.Count; i++) {
			...
			if (j == 0 || i == faceData.Count) {
				if (i != 0) {
					...
					newGO.transform.parent = returnGO.transform;

					newGO.AddComponent<MeshFilter> ().mesh = mesh;
					newGO.AddComponent<MeshRenderer> ();
					Debug.Log ("Added mesh with " + length + "vertexes.");
				}

			// Make new vectors
				if (i != faceData.Count && i / MAX_VERT == faceData.Count / MAX_VERT) {
			...
			if (i != faceData.Count) {
				...
				newTriangles [j] = triangles[i] - length;
			}
		}
		return returnGO;
	}
