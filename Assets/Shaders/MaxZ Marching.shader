...
Shader "Hidden/Ray Marching/MaxZ" 
{
	...
	half4 raymarch(v2f i, float offset) 
	{
		...
		float zCutoff = floor(_numSlices/2);
			
		for(int k = 0; k < STEP_CNT; k++)
		{
			pos = frontPos + k * stepDist;

			pos.z = floor(pos.z * 128.0f)/128.0f;
			float i = floor(pos.z * (_numSlices-1.0f)); 
			...
			float x = xStart + 0.5f + floor(pos.x * (_tPixelWidth-1));
			float y = yStart + 0.5f + floor(pos.y * (_tPixelHeight-1)); // could be wrong?
			...
			float oldMeanColor = (dst.r + dst.g + dst.b) / 3.0f;

			if (meanColor < _Threshold){
				meanColor = 0.0f;
			}

			if (meanColor > oldMeanColor){
				dst = src;
			}
