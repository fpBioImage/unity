...
Shader "Hidden/Ray Marching/Ray Marching Top" 
{
	...
	#define TOTAL_STEPS 512.0
	#define STEP_CNT 512
	#define STEP_SIZE 1 / 511.0
	...
	half4 raymarch(v2f i, float offset) 
	{
		...
		for(int k = 0; k < STEP_CNT; k++)
		{
			...
			//float roundZ = floor(pos.z * 511.0f)/511.0f;
			//float i = floor(roundZ * (_numSlices-1.0f)); 
			...
			float x = xStart + pos.x * (_tPixelWidth-1);
			float y = yStart + pos.y * (_tPixelHeight-1); // could be wrong?
			...
			// Smoothing:
			src.a *= meanColor; // is this helping
